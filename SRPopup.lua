...
function M.new()
	...
	local is_admin
	local is_deleting
	local is_raid_locked = false
	local is_showing_absentees = false
	...
	local function on_lock_click()
		m.msg.lock_sr( m.db.events[ event_id ].sr.reference, not is_raid_locked )
		this:Disable()
	end

	local function on_check_absentees()
		if is_showing_absentees then
			sr_list = m.db.events[ event_id ].sr.reservations
			popup.admin_status:SetText( "" )
			refresh_list()
			popup.btn_check_absentees:SetText( m.L( "actions.check_absentees" ) )
			popup.btn_admin_remove:Hide()
			is_deleting = false
			is_showing_absentees = false
			return
		end
		...
		popup.admin_status:SetText( m.L( "ui.showing_absentees", { count = getn( missing ) } ) )
		sr_list = missing
		is_deleting = true
		is_showing_absentees = true
		refresh_list()
		popup.btn_check_absentees:SetText( m.L( "actions.show_all" ) )
	...
	local function on_remove_absentees()
		...
		popup.admin_status:SetText( m.L( "ui.deleting_srs_left", { count = getn( sr_list ) } ) )
	...
	local function create_frame()
		...
		label_locked:SetText( m.L( "ui.raid_locked" ) )
		...
		label_yoursr:SetText( m.L( "ui.your_reservations", { count = 0, limit = 0 } ) )
		...
		frame.dd_sr1 = scroll_drop:New( border_reserve, {
			default_text = m.L( "ui.reserve_item" ),
			dropdown_style = m.pfui_skin_enabled and "pfui" or "classic",
			label_on_select = "text",
			search = true,
			width = 200,
		...
		frame.label_comment:SetText( m.L( "ui.comment" ) )
		...
		frame.btn_lock = gui.create_button( border_admin, m.L( "actions.lock_raid" ), nil, on_lock_click )
		frame.btn_lock:SetPoint( "TopRight", border_admin, "TopRight", -5, -5 )

		frame.btn_check_absentees = gui.create_button( border_admin, m.L( "actions.check_absentees" ), 110, on_check_absentees )
	...
	function refresh()
		...
		if not m.db.events[ event_id ].sr then
			popup.yoursr:SetText( m.L( "ui.loading_sr" ) )
			popup.dd_spec:Hide()
			popup.label_sr1:Hide()
			popup.dd_sr1:Hide()
			popup.label_sr2:Hide()
		...
		popup.titlebar.title:SetText( m.L( "ui.sr_for_title", { event = m.db.events[ event_id ].title, raid = m.loot_table[ raid_id ].name } ) )
		...
		if sr.adminAccess and m.find( m.player, m.db.user_settings.sr_admins ) then
			...
			if sr.locked then
				is_raid_locked = true
				popup.btn_lock:SetText( m.L( "actions.unlock_raid" ) )
			else
				is_raid_locked = false
				popup.btn_lock:SetText( m.L( "actions.lock_raid" ) )
			end
		else
			is_admin = false
