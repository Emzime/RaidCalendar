...
function M.new()
	...
	local function get_cell()
		...
		if cell_pool_n > 0 then
			...
			cell.reset_icon:SetPoint( "BottomRight", cell, "BottomRight", -3, 3 )
	...
	local function render_calendar()
		...
		for i = 1, TOTAL_CELLS do
			...
			if m.should_show_raid_reset_icons and m.should_show_raid_reset_icons() and cell.raid_resets and getn( cell.raid_resets ) > 0 then
				cell.reset_icon:Show()
			else
				cell.reset_icon:Hide()
			end
			cell:Show()
	...
	local function create_frame()
		...
		cb_reset_icons:SetPoint( "TopLeft", settings, "TopLeft", settings_label_x, settings_first_row_y - (settings_row_spacing * 3) + 4 )
